...
- Reject PRs that add skills into source code/repo content directly (for example under `skills/` or seed-only additions intended as published skills), unless maintainers explicitly approve the curated public-corpus exception in `specs/dev-seeding.md`. Ordinary community/publisher skills must be uploaded/published via CLI.
