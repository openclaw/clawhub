...
- **Before writing or reviewing Convex queries, check deployment health.** Use the Convex MCP `insights` tool (or `mcp__convex__insights`) to check for OCC conflicts, `bytesReadLimit`, and `documentsReadLimit` errors. Use the `logs` tool with `status: "failure"` to see individual error messages and stack traces. This helps identify which functions are causing bandwidth issues so you can prioritize fixes.
