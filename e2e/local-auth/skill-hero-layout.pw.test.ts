...
test.skip(
  ...
  "skill hero layout requires the seeded local Convex fixture",
);

test.beforeAll(() => {
  const result = spawnSync(
    "bunx",
    ["convex", "run", "--no-push", "devSeed:seedCanonicalSearchFixture"],
...
test("skill hero metadata keeps semantic wrap groups on mobile", async ({ page }) => {
  ...
  const initialTopicCount = await topics.count();
  ...
  await expect(topics).toHaveCount(initialTopicCount + 2);
