...
describe("github publish source helpers", () => {
  ...
  it("retries public GitHub fetches without a scoped token", async () => {
    const archiveBytes = zipSync({
      "repo-root/package.json": new TextEncoder().encode('{"name":"demo","version":"1.0.0"}\n'),
      "repo-root/openclaw.plugin.json": new TextEncoder().encode(
        '{"id":"demo","configSchema":{"type":"object"}}\n',
    ...
    const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (url.endsWith("/repos/owner/repo/zipball/0123456789abcdef0123456789abcdef01234567")) {
    ...
    try {
      ...
      expect(fetched.source.commit).toBe("0123456789abcdef0123456789abcdef01234567");
      expect(await readFile(join(fetched.dir, "openclaw.plugin.json"), "utf8")).toContain(
        '"id":"demo"',
      );
