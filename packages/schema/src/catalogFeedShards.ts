...
export const CATALOG_FEED_SHARD_ROOT_PAYLOAD_TYPE =
  "openclaw.official-external-plugin-catalog-shard-root.v1";
export const CATALOG_SKILLS_FEED_SHARD_ROOT_PAYLOAD_TYPE =
  "openclaw.official-skills-catalog-shard-root.v1";
...
function requireFeedId(feedId: string) {
  if (feedId !== CATALOG_FEED_ID && feedId !== CATALOG_SKILLS_FEED_ID) {
    throw new Error("Catalog feed shard feedId is unsupported");
  }
}

export function parseCatalogFeedShardRoot(value: unknown): CatalogFeedShardRoot {
  ...
  for (const [index, shard] of root.shards.entries()) {
    if (shard.index !== index) throw new Error("Catalog feed shard indexes must be contiguous");
    if (!/^[a-f0-9]{64}$/u.test(shard.sha256)) {
      throw new Error("Catalog feed shard digest must be lowercase SHA-256 hex");
    }
    if (digests.has(shard.sha256)) throw new Error("Catalog feed shard digest is duplicated");
    digests.add(shard.sha256);
    if (
      !Number.isSafeInteger(shard.byteLength) ||
      shard.byteLength < 1 ||
    ...
    if (
      !Number.isSafeInteger(shard.entryCount) ||
      shard.entryCount < 1 ||
    ...
    if (urls.has(url.href)) throw new Error("Catalog feed shard URL is duplicated");
    urls.add(url.href);
...
export function parseCatalogFeedShard(value: unknown): CatalogFeedShard {
...
export function serializeCatalogFeedShard(shard: CatalogFeedShard): string {
...
export function serializeCatalogFeedShardRoot(root: CatalogFeedShardRoot): string {
...
export async function validateCatalogFeedShardSet(
  rootValue: unknown,
  shardPayloads: readonly string[],
): Promise<{
  ...
  const shards = await Promise.all(
    shardPayloads.map(async (payload, index) => {
