...
function categoryTokenMatchesKeyword(token, keyword) {
    if (token === keyword)
        return true;
    if (keyword === "dev") {
...
function stripGeneratedSlugPrefixTokens(tokens) {
    if (tokens[0] !== "dev")
        return tokens;
...
export function deriveSkillPrimaryCategory(skill) {
    ...
    for (const category of SKILL_CATEGORY_DEFINITIONS) {
        const score = category.keywords.reduce((total, keyword) => {
            const primaryScore = primaryTokens.some((token) => categoryTokenMatchesKeyword(token, keyword))
                ? 2
                : 0;
...
export function resolveSkillPrimaryCategory(skill) {
    if (isSkillCategorySlug(skill.primaryCategory))
        return skill.primaryCategory;
...
export function normalizeInferredCatalogTopics(values) {
    ...
    for (const value of values ?? []) {
        const topic = normalizeCatalogTopic(value);
        if (!topic || topic.length > CATALOG_TOPIC_MAX_LENGTH || seen.has(topic))
            continue;
        seen.add(topic);
        normalized.push(topic);
        if (normalized.length === CATALOG_TOPIC_LIMIT)
