...
import { CatalogFeedEntrySchema, CatalogFeedStateSchema, CATALOG_FEED_SCHEMA_VERSION, } from "./catalogFeed.js";
export const CATALOG_FEED_QUERY_PAYLOAD_TYPE = "openclaw.official-external-plugin-catalog-query-results.v1";
export const CATALOG_FEED_CHANGES_PAYLOAD_TYPE = "openclaw.official-external-plugin-catalog-changes.v1";
...
export const CatalogFeedChangeSchema = type(CatalogFeedUpsertChangeSchema.or(CatalogFeedRemoveChangeSchema).or(CatalogFeedMetadataChangeSchema));
...
export function normalizeCatalogFeedQuery(value) {
    ...
    if (query.types !== undefined) {
        const types = sortedUnique(query.types);
        if (types.length === 0)
            throw new Error("Catalog feed query types must not be empty");
        normalized.types = types;
    }
    if (query.states !== undefined) {
        const states = sortedUnique(query.states);
        if (states.length === 0)
            throw new Error("Catalog feed query states must not be empty");
...
function queriesEqual(left, right) {
    const arraysEqual = (first, second) => first === undefined
        ? second === undefined
        : second !== undefined &&
...
export function parseCatalogFeedQueryPage(value) {
...
export function parseCatalogFeedQueryPages(values) {
    if (values.length === 0) {
...
export function parseCatalogFeedChangePage(value) {
    ...
    for (const change of page.changes) {
        ...
        if (change.sequence > priorSequence + 1 &&
        ...
        if (change.operation === "remove") {
            ...
            requireBoundedString(change.metadata.description, "Catalog feed metadata description", CATALOG_FEED_DESCRIPTION_MAX_BYTES);
        }
    }
...
export function parseCatalogFeedChangePages(values) {
    if (values.length === 0) {
...
export function parseCatalogFeedResetRequired(value) {
...
//# sourceMappingURL=catalogFeedDistribution.js.map