...
describe("cmdRescanAllPackages", () => {
  it("respects max-packages including skips and returns the resume cursor", async () => {
    httpMocks.apiRequest.mockResolvedValueOnce({
      ok: true,
      mode: "all-active-latest",
      queued: 1,
      ...
      sampleNames: ["one", "deleted"],
    });
    const result = await cmdRescanAllPackages(
      makeGlobalOpts(),
      { dryRun: true, maxPackages: 2, batchSize: 10, cursor: "start" },
      false,
    );
    expect(httpMocks.apiRequest).toHaveBeenCalledTimes(1);
    expect(httpMocks.apiRequest).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ body: expect.objectContaining({ batchSize: 2, cursor: "start" }) }),
      expect.anything(),
    );
    expect(result).toMatchObject({ queued: 1, skipped: 1, cursor: "resume" });
  });

  it.each(["failed", "missing"])(
    "stops at a drained batch with %s jobs when fail-fast is set",
    async (kind) => {
      httpMocks.apiRequest
        .mockResolvedValueOnce({
          ok: true,
        ...
        .mockResolvedValueOnce({
          ...
          missing: kind === "missing" ? 1 : 0,
          terminal: 1,
          done: true,
          failedJobIds: kind === "failed" ? ["job-1"] : [],
        });
      await expect(
        cmdRescanAllPackages(
          makeGlobalOpts(),
          { yes: true, failFast: true, pollInterval: 0 },
          false,
        ),
      ).rejects.toThrow(/failed or missing/);
      expect(httpMocks.apiRequest).toHaveBeenCalledTimes(2);
    },
  );

  it("requires --yes for real runs when input is disabled", async () => {
    await expect(cmdRescanAllPackages(makeGlobalOpts(), {}, false)).rejects.toThrow(/--yes/i);
    expect(httpMocks.apiRequest).not.toHaveBeenCalled();
  });
  ...
  it("pages dry-runs without polling status", async () => {
    ...
    expect(httpMocks.apiRequest).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        path: "/api/v1/packages/-/scan/batch",
        body: {
          mode: "all-active-latest",
          cursor: null,
          batchSize: 2,
  ...
  it("queues a batch and polls until drained", async () => {
    ...
    expect(httpMocks.apiRequest).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        path: "/api/v1/packages/-/scan/batch/status",
        token: "tkn",
        body: { jobIds: ["securityScanJobs:1"] },
      }),
      expect.anything(),
  ...
  it("fails when drained batches contain failed jobs", async () => {
    ...
    await expect(
      cmdRescanAllPackages(makeGlobalOpts(), { yes: true, batchSize: 1, pollInterval: 0 }, false),
    ).rejects.toThrow(/failed job/i);
  });
});
