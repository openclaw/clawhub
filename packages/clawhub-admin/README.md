...
bun run admin -- skills rescan-all [--mode all-active-latest|truncation-risk-latest] [--dry-run] [--batch-size <n>] [--cursor <cursor>] [--json]
...
```bash
bun run admin -- skills rescan-all --mode truncation-risk-latest --dry-run --json
```
