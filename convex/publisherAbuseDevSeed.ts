...
async function seedTemporalCohortDemoRows(ctx: ClearSeedCtx, args: { now: number }) {
  ...
  await ctx.db.insert("publisherAbuseSignalCommunications", {
    ...
    request: {
      ...
      state: "sent",
      latestAttemptAt: temporalCompletedAt - 4 * 60_000,
      attemptCount: 1,
      sentAt: temporalCompletedAt - 4 * 60_000,
      recipientUserId: temporalUserId,
      recipientEmail: "local-abuse@openclaw.invalid",
      providerId: "demo-email-sent",
      subject: "Question about downloads for Demo Temporal Download Burst",
  ...
  const communicationStateSignals = [
    {
      skillDisplayName: "Demo Communication — Queued",
      contactState: "queued" as const,
      attentionState: "not_contacted" as const,
      needsAttention: false,
      request: {
        requestedAt: temporalCompletedAt - 50 * 60_000,
        tokenHash: "ffe054fe7ae0cb6dc65c3af9b61d5209f439851db43d0ba5997337df154668eb",
    ...
    {
      skillDisplayName: "Demo Communication — Retrying",
      contactState: "retrying" as const,
      attentionState: "not_contacted" as const,
      needsAttention: false,
      request: {
        requestedAt: temporalCompletedAt - 45 * 60_000,
        tokenHash: "ffe054fe7ae0cb6dc65c3af9b61d5209f439851db43d0ba5997337df154668eb",
        state: "retrying" as const,
        latestAttemptAt: temporalCompletedAt - 20 * 60_000,
        attemptCount: 2,
        recipientUserId: temporalUserId,
        recipientEmail: "local-abuse@openclaw.invalid",
        subject: "Question about downloads for Demo Communication — Retrying",
        templateVersion: "traffic-explanation.v1",
        reasonBullets: ["Downloads rose unusually quickly while installs stayed flat."],
        redactedTextSnapshot: "Hello local-abuse,\n\n[SECURE EXPLANATION LINK]",
        deliveryError: "resend_error",
      },
    },
    {
      skillDisplayName: "Demo Communication — Awaiting owner",
      contactState: "sent" as const,
      attentionState: "awaiting_owner" as const,
      needsAttention: false,
      request: {
        requestedAt: temporalCompletedAt - 42 * 60_000,
        tokenHash: "ffe054fe7ae0cb6dc65c3af9b61d5209f439851db43d0ba5997337df154668eb",
        state: "sent" as const,
        latestAttemptAt: temporalCompletedAt - 37 * 60_000,
        attemptCount: 1,
        sentAt: temporalCompletedAt - 37 * 60_000,
        recipientUserId: temporalUserId,
        recipientEmail: "local-abuse@openclaw.invalid",
        providerId: "demo-email-awaiting-owner",
        subject: "Question about downloads for Demo Communication — Awaiting owner",
        templateVersion: "traffic-explanation.v1",
        reasonBullets: ["Downloads rose unusually quickly while installs stayed flat."],
        redactedTextSnapshot: "Hello local-abuse,\n\n[SECURE EXPLANATION LINK]",
      },
    ...
    {
      skillDisplayName: "Demo Communication — Not deliverable",
      contactState: "not_deliverable" as const,
      attentionState: "contact_failed" as const,
      needsAttention: true,
      ...
      request: {
        requestedAt: temporalCompletedAt - 40 * 60_000,
        state: "not_deliverable" as const,
        latestAttemptAt: temporalCompletedAt - 10 * 60_000,
        attemptCount: 4,
        recipientUserId: temporalUserId,
        recipientEmail: "local-abuse@openclaw.invalid",
        subject: "Question about downloads for Demo Communication — Not deliverable",
        templateVersion: "traffic-explanation.v1",
        reasonBullets: ["Downloads rose unusually quickly while installs stayed flat."],
        redactedTextSnapshot: "Hello local-abuse,\n\n[SECURE EXPLANATION LINK]",
        deliveryError: "resend_error",
    ...
    {
      skillDisplayName: "Demo Communication — Staff alert failed",
      contactState: "sent" as const,
      attentionState: "needs_attention" as const,
      needsAttention: true,
      notificationEventKind: "publisher_abuse_signal_owner_response_submitted" as const,
      notificationState: "failed" as const,
      notificationAttemptCount: 4,
      notificationLatestAttemptAt: temporalCompletedAt - 60_000,
      lastNotificationError: "Hermit publisher abuse event failed: 503 unavailable",
      needsNotification: false,
      request: {
        requestedAt: temporalCompletedAt - 35 * 60_000,
        tokenHash: "ffe054fe7ae0cb6dc65c3af9b61d5209f439851db43d0ba5997337df154668eb",
        state: "sent" as const,
        latestAttemptAt: temporalCompletedAt - 30 * 60_000,
        attemptCount: 1,
        sentAt: temporalCompletedAt - 30 * 60_000,
        recipientUserId: temporalUserId,
        recipientEmail: "local-abuse@openclaw.invalid",
        providerId: "demo-email-alert-failed",
        subject: "Question about downloads for Demo Communication — Staff alert failed",
        templateVersion: "traffic-explanation.v1",
        reasonBullets: ["Downloads rose unusually quickly while installs stayed flat."],
        redactedTextSnapshot: "Hello local-abuse,\n\n[SECURE EXPLANATION LINK]",
      },
      response: {
        ...
        submittedAt: temporalCompletedAt - 25 * 60_000,
        submittedByUserId: temporalUserId,
      },
    },
  ];
  for (const [index, state] of communicationStateSignals.entries()) {
    const signalId = await ctx.db.insert("publisherAbuseSignals", {
      ...
      skillId: temporalSkillId,
      skillSlug: TEMPORAL_DEMO_SKILL_SLUG,
      skillDisplayName: state.skillDisplayName,
      latestRunId: temporalRunId,
      latestScoreId: temporalScoreId,
      firstSeenAt: temporalCompletedAt - (index + 2) * 60_000,
      lastSeenAt: temporalCompletedAt - (index + 1) * 60_000,
      seenCount: 1,
      recent7Downloads: temporalDownloads7d,
      recent7Installs: temporalInstalls7d,
      recent7InstallDownloadRatio: temporalInstalls7d / temporalDownloads7d,
      ...
      ...(state.notificationEventKind
        ? {
            ...
            notificationAttemptCount: state.notificationAttemptCount,
            notificationLatestAttemptAt: state.notificationLatestAttemptAt,
            lastNotificationError: state.lastNotificationError,
