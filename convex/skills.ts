...
import { buildModerationSnapshot, resolveSkillVerdict } from "./lib/moderationEngine";
import {
...
export const getBySlug = query({
  ...
  handler: async (ctx, args) => {
    ...
    const verdict = resolveSkillVerdict(skill);
    const isMalwareBlocked =
      verdict === "malicious" || (skill.moderationFlags?.includes("blocked.malware") ?? false);
    const isSuspicious =
      verdict === "suspicious" || (skill.moderationFlags?.includes("flagged.suspicious") ?? false);
    const isHiddenByMod =
...
/**
 * Cursor-based skill scan for v2 moderation backfill.
 * Includes active + hidden skills, excludes hard-removed skills.
 */
export const getSkillBatchForModerationBackfillInternal = internalQuery({
  args: {
    cursor: v.optional(v.number()),
    batchSize: v.optional(v.number()),
  },
...
export const applyModerationBackfillInternal = internalMutation({
  args: {
    skillId: v.id("skills"),
  ...
  handler: async (ctx, args) => {
    ...
    if (!skill || !version) return { ok: false as const, reason: "missing" };
    if ((skill.moderationStatus ?? "active") === "removed") {
    ...
    const snapshot = buildModerationSnapshot({
      ...
      existingReasonCodes: (skill.moderationReasonCodes as string[] | undefined) ?? [],
      existingEvidence: (skill.moderationEvidence as ModerationFinding[] | undefined) ?? [],
      sourceVersionId: version._id,
    });

    const now = Date.now();
    const nextStatus =
      snapshot.verdict === "malicious" && (skill.moderationStatus ?? "active") === "active"
        ? "hidden"
        : skill.moderationStatus;

    await ctx.db.patch(skill._id, {
      moderationStatus: nextStatus,
      moderationFlags: snapshot.legacyFlags,
      ...
      hiddenAt:
        nextStatus === "hidden" && (skill.moderationStatus ?? "active") === "active"
          ? now
          : skill.hiddenAt,
      updatedAt: now,
    });
    return { ok: true as const, verdict: snapshot.verdict };
  },
});
...
export const approveSkillByHashInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (skill) {
      ...
      const existingReasonCodes = (skill.moderationReasonCodes as string[] | undefined) ?? [];
      const existingEvidence = (skill.moderationEvidence as ModerationFinding[] | undefined) ?? [];
      const snapshot = buildModerationSnapshot({
        existingReasonCodes,
        existingEvidence,
        vtStatus: scanner === "vt" ? args.status : undefined,
        llmStatus: scanner === "llm" ? args.status : undefined,
        sourceVersionId: version._id,
      });
      ...
      const nextSummary = summarizeReasonCodes(nextReasonCodes);
  ...
  handler: async (ctx, args) => {
    ...
    const nextReasonCodes =
      bypassSuspicious && !isMalicious
        ...
        : snapshot.reasonCodes;
    const nextVerdict = verdictFromCodes(nextReasonCodes);
    patch.moderationVerdict = nextVerdict;
    patch.moderationReasonCodes = nextReasonCodes.length ? nextReasonCodes : undefined;
    patch.moderationEvidence = snapshot.evidence.length ? snapshot.evidence : undefined;
    patch.moderationSummary = summarizeReasonCodes(nextReasonCodes);
