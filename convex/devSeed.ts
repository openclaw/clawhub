...
async function seedLocalFixturesHandler(
  ...
  const publisherSwitcherResult: SeedMutationResult = await ctx.runMutation(
    internal.devSeed.seedLocalPublisherSwitcherContentMutation,
    {
      reset: args.reset,
    },
  );
  ...
  return {
    ...
    results: [
      ...
      { slug: "local-publisher-switcher-content", ...publisherSwitcherResult },
    ],
  };
}
...
type LocalPublisherSwitcherItemSpec = {
  kind: "skill" | "plugin";
  slugOrName: string;
  displayName: string;
  summary: string;
...
const LOCAL_PUBLISHER_SWITCHER_FIXTURES: Array<{
  handle: string;
  displayName: string;
  items: LocalPublisherSwitcherItemSpec[];
}> = [
  {
    handle: LOCAL_OFFICIAL_MEMBER.handle,
    displayName: LOCAL_OFFICIAL_MEMBER.displayName,
    items: [
      {
        kind: "skill",
        slugOrName: "member-inbox-triage",
        displayName: "Member Inbox Triage",
        summary: "Turn messy personal publisher requests into a short action queue.",
        version: "0.2.0",
        ageMs: 2 * 60 * 60 * 1000,
        categories: ["productivity"],
        topics: ["inbox", "triage", "publishing"],
        stats: { downloads: 42, installs: 12, stars: 4 },
      },
      {
        kind: "skill",
        slugOrName: "member-release-notes",
        displayName: "Member Release Notes",
        summary: "Draft concise release notes from local package changes.",
        version: "0.1.3",
        ageMs: 19 * 60 * 60 * 1000,
        categories: ["documentation"],
        topics: ["releases", "copy", "workflow"],
        stats: { downloads: 31, installs: 9, stars: 3 },
      },
      {
        kind: "plugin",
        slugOrName: "member-sandbox-runner",
        displayName: "Member Sandbox Runner",
        summary: "Run lightweight package checks from a personal workspace.",
        version: "0.1.0",
        ageMs: 28 * 60 * 60 * 1000,
        categories: ["runtime"],
        topics: ["sandbox", "validation", "local"],
        stats: { downloads: 58, installs: 15, stars: 5 },
      },
    ],
  },
  {
    handle: LOCAL_OFFICIAL_ORG.handle,
    displayName: LOCAL_OFFICIAL_ORG.displayName,
    items: [
      {
        kind: "skill",
        slugOrName: "org-release-checklist",
        displayName: "Org Release Checklist",
        summary: "Keep team release reviews consistent before publishing.",
        version: "1.1.0",
        ageMs: 5 * 60 * 60 * 1000,
        categories: ["operations"],
        topics: ["release", "review", "team"],
        stats: { downloads: 188, installs: 64, stars: 18 },
      },
      {
        kind: "skill",
        slugOrName: "org-support-escalation",
        displayName: "Org Support Escalation",
        summary: "Route publisher support cases with severity and ownership context.",
        version: "0.4.0",
        ageMs: 31 * 60 * 60 * 1000,
        categories: ["support"],
        topics: ["support", "routing", "publisher"],
        stats: { downloads: 156, installs: 42, stars: 14 },
      },
      {
        kind: "plugin",
        slugOrName: "org-audit-reporter",
        displayName: "Org Audit Reporter",
        summary: "Bundle publisher audit signals into a team-facing report.",
        version: "0.3.0",
        ageMs: 7 * 60 * 60 * 1000,
        categories: ["security"],
        topics: ["audit", "reporting", "compliance"],
        stats: { downloads: 244, installs: 73, stars: 22 },
      },
    ],
  },
];

async function ensureLocalOfficialPublisherContexts(ctx: MutationCtx) {
  const now = Date.now();
  const existingUser = await ctx.db
    .query("users")
    .withIndex("handle", (q) => q.eq("handle", LOCAL_OFFICIAL_MEMBER.handle))
    .unique();
  const userPatch = {
    handle: LOCAL_OFFICIAL_MEMBER.handle,
    displayName: LOCAL_OFFICIAL_MEMBER.displayName,
    name: LOCAL_OFFICIAL_MEMBER.displayName,
    role: "user" as const,
    githubCreatedAt: LOCAL_SEED_GITHUB_CREATED_AT,
    deletedAt: undefined,
    deactivatedAt: undefined,
  ...
  if (existingUser) await ctx.db.patch(existingUser._id, userPatch);

  const user = await ctx.db.get(userId);
  if (!user) throw new Error("Local official member was not created");
  const memberPublisher = await ensurePersonalPublisherForUser(ctx, user, {
    actorUserId: user._id,
    source: "dev_seed.publisher_switcher_content",
  });
  if (!memberPublisher) throw new Error("Local official member publisher was not created");

  let orgPublisher = await getPublisherByHandle(ctx, LOCAL_OFFICIAL_ORG.handle);
  let orgPublisherId = orgPublisher?._id;
  if (!orgPublisherId) {
    ...
    orgPublisher = await ctx.db.get(orgPublisherId);
  } else {
    await ctx.db.patch(orgPublisherId, {
      displayName: LOCAL_OFFICIAL_ORG.displayName,
      trustedPublisher: true,
      deletedAt: undefined,
      deactivatedAt: undefined,
  ...
  if (!orgPublisherId) throw new Error("Local official org publisher was not created");

  const official = await ctx.db
    .query("officialPublishers")
    .withIndex("by_publisher", (q) => q.eq("publisherId", orgPublisherId))
    .unique();
  if (official) {
    await ctx.db.patch(official._id, {
      reason: LOCAL_OFFICIAL_ORG.reason,
  ...
  const membership = await getPublisherMembership(ctx, orgPublisherId, userId);
  if (membership) {
  ...
  return {
    userId,
    contexts: {
      [LOCAL_OFFICIAL_MEMBER.handle]: {
        userId,
        publisherId: memberPublisher._id,
      },
      [LOCAL_OFFICIAL_ORG.handle]: {
        userId,
        publisherId: orgPublisherId,
      },
    },
...
function localPublisherSwitcherSkillMd(spec: LocalPublisherSwitcherItemSpec) {
  return `---
name: ${spec.slugOrName}
...
async function seedLocalPublisherSwitcherSkill(
  ctx: MutationCtx,
  params: {
    userId: Id<"users">;
    publisherId: Id<"publishers">;
    spec: LocalPublisherSwitcherItemSpec;
    now: number;
    reset?: boolean;
  },
) {
  const existing = await ctx.db
    .query("skills")
    .withIndex("by_slug", (q) => q.eq("slug", params.spec.slugOrName))
    .unique();
  if (existing && params.reset) {
    await deleteSkillAndVersions(ctx, existing._id);
  } else if (existing) {
    await ctx.db.patch(existing._id, {
      displayName: params.spec.displayName,
      summary: params.spec.summary,
      ownerUserId: params.userId,
      ownerPublisherId: params.publisherId,
      categories: params.spec.categories,
      topics: params.spec.topics,
      moderationStatus: "active",
      moderationReason: "clean",
      moderationVerdict: "clean",
      moderationFlags: [],
      isSuspicious: false,
      statsDownloads: params.spec.stats.downloads,
      statsStars: params.spec.stats.stars,
      statsInstallsCurrent: params.spec.stats.installs,
      statsInstallsAllTime: params.spec.stats.installs,
      stats: {
        downloads: params.spec.stats.downloads,
        installsCurrent: params.spec.stats.installs,
        installsAllTime: params.spec.stats.installs,
        stars: params.spec.stats.stars,
        versions: existing.stats?.versions ?? 1,
        comments: existing.stats?.comments ?? 0,
      },
      updatedAt: params.now,
    });
    await ensureHighlightedSkillBadge(ctx, existing._id, params.userId, params.now);
    await ensurePublicCorpusSkillDailyStats(ctx, {
      skillId: existing._id,
      key: params.spec.slugOrName,
      downloads: params.spec.stats.downloads,
      installs: params.spec.stats.installs,
      now: params.now,
    });
    return existing._id;
  }

  const createdAt = params.now - params.spec.ageMs;
  const skillMd = localPublisherSwitcherSkillMd(params.spec);
  const frontmatter = parseFrontmatter(skillMd);
  const skillId = await ctx.db.insert("skills", {
    slug: params.spec.slugOrName,
    displayName: params.spec.displayName,
    summary: params.spec.summary,
    ownerUserId: params.userId,
    ownerPublisherId: params.publisherId,
    ...
    softDeletedAt: undefined,
    badges: {
      highlighted: { byUserId: params.userId, at: params.now },
  ...
  await ensureHighlightedSkillBadge(ctx, skillId, params.userId, params.now);

  const versionId = await ctx.db.insert("skillVersions", {
    skillId,
    version: params.spec.version,
    changelog: "Seeded local publisher switcher fixture.",
    changelogSource: "user",
    files: [],
    parsed: {
      frontmatter,
      metadata: {},
      clawdis: parseClawdisMetadata(frontmatter),
    },
  ...
  await ctx.db.patch(skillId, {
    ...
    latestVersionSummary: {
      ...
      changelog: "Seeded local publisher switcher fixture.",
      changelogSource: "user",
      description: params.spec.summary,
      clawdis: parseClawdisMetadata(frontmatter),
    },
    tags: { latest: versionId },
    stats: {
...
async function seedLocalPublisherSwitcherPlugin(
  ctx: MutationCtx,
  params: {
    userId: Id<"users">;
    publisherId: Id<"publishers">;
  ...
  const normalizedName = normalizePackageName(params.spec.slugOrName);
  const existing = await ctx.db
    .query("packages")
    .withIndex("by_name", (q) => q.eq("normalizedName", normalizedName))
    .unique();
  if (existing && params.reset) {
    ...
    await ctx.db.patch(existing._id, {
      ...
      ...seededPackageRecommendationPatch(params.spec.stats),
      updatedAt: params.now,
    });
    await ensureHighlightedPackageBadge(ctx, existing._id, params.userId, params.now);
    const updatedPackage = await ctx.db.get(existing._id);
    if (updatedPackage) {
      await upsertPackageSearchDigest(ctx, extractPackageDigestFields(updatedPackage));
    }
  ...
  const verification = {
    ...
    scope: "artifact-only" as const,
    summary: "Seeded local publisher switcher fixture.",
    sourceRepo: `openclaw/${normalizedName}`,
    scanStatus: "clean" as const,
  };
  const packageId = await ctx.db.insert("packages", {
    name: params.spec.slugOrName,
    normalizedName,
    displayName: params.spec.displayName,
    summary: params.spec.summary,
    ownerUserId: params.userId,
  ...
  const releaseId = await ctx.db.insert("packageReleases", {
    ...
    distTags: ["latest"],
    files: [],
    integritySha256: `local-publisher-switcher-${normalizedName}`,
    extractedPackageJson: {
      name: params.spec.slugOrName,
      version: params.spec.version,
      description: params.spec.summary,
    ...
    compatibility,
    verification,
    sha256hash: `local-publisher-switcher-${normalizedName}`,
    vtAnalysis: {
      status: "clean",
      verdict: "clean",
      analysis: "Local publisher switcher fixture scanned clean.",
    ...
    llmAnalysis: {
      ...
      verdict: "clean",
      confidence: "high",
      summary: "Local publisher switcher fixture is safe sample content.",
    ...
    staticScan: {
      ...
      reasonCodes: [],
      findings: [],
      summary: "Local publisher switcher fixture static scan clean.",
      engineVersion: "local-dev-fixture",
      checkedAt: params.now,
    },
    source: { kind: "github", repo: `openclaw/${normalizedName}`, path: "." },
    createdBy: params.userId,
    publishActor: { kind: "user", userId: params.userId },
    createdAt,
    softDeletedAt: undefined,
  });
  await ctx.db.patch(packageId, {
  ...
  await ensureHighlightedPackageBadge(ctx, packageId, params.userId, params.now);
  const packageDoc = await ctx.db.get(packageId);
  if (packageDoc) {
    await upsertPackageSearchDigest(ctx, extractPackageDigestFields(packageDoc));
  }
...
async function refreshLocalPublisherSwitcherCounts(
  ctx: MutationCtx,
  publisherId: Id<"publishers">,
) {
  const skills = await ctx.db
  ...
  const packages = await ctx.db
    ...
    .withIndex("by_owner_publisher", (q) => q.eq("ownerPublisherId", publisherId))
    .collect();
  const skillTotals = skills.reduce(
    (totals, skill) => {
      totals.downloads += readCanonicalStat(skill, "downloads");
      totals.installs += readCanonicalStat(skill, "installsAllTime");
      totals.stars += readCanonicalStat(skill, "stars");
      return totals;
    },
  ...
  const packageTotals = packages.reduce(
    (totals, pkg) => {
      totals.downloads += pkg.stats.downloads;
      totals.installs += pkg.stats.installs;
      totals.stars += pkg.stats.stars;
      return totals;
    },
    { downloads: 0, installs: 0, stars: 0 },
  );
  await ctx.db.patch(publisherId, {
    publishedSkills: skills.filter((skill) => !skill.softDeletedAt).length,
    publishedPackages: packages.filter((pkg) => !pkg.softDeletedAt).length,
    skillTotalDownloads: skillTotals.downloads,
    skillTotalInstalls: skillTotals.installs,
    skillTotalStars: skillTotals.stars,
    totalDownloads: skillTotals.downloads + packageTotals.downloads,
    totalInstalls: skillTotals.installs + packageTotals.installs,
    totalStars: skillTotals.stars + packageTotals.stars,
    updatedAt: Date.now(),
  });
}

export const seedLocalPublisherSwitcherContentMutation = internalMutation({
  args: {
    reset: v.optional(v.boolean()),
  },
  handler: async (ctx, args) => {
    const now = Date.now();
    const { userId, contexts } = await ensureLocalOfficialPublisherContexts(ctx);
    const seeded: string[] = [];

    for (const fixture of LOCAL_PUBLISHER_SWITCHER_FIXTURES) {
      const owner = contexts[fixture.handle];
      if (!owner) throw new Error(`Missing local publisher context: ${fixture.handle}`);
      for (const spec of fixture.items) {
        if (spec.kind === "skill") {
          await seedLocalPublisherSwitcherSkill(ctx, {
            userId,
          ...
          await seedLocalPublisherSwitcherPlugin(ctx, {
            userId,
            publisherId: owner.publisherId,
            spec,
            now,
        ...
        seeded.push(`${fixture.handle}:${spec.kind}:${spec.slugOrName}`);
      }
      await refreshLocalPublisherSwitcherCounts(ctx, owner.publisherId);
    }
...
function flaggedWalletSkillSpectorAnalysis(
  ...
  return {
    ...
    severity: "critical",
    recommendation:
      "Block publishing and review the sensitive-data handling before restoring visibility.",
    issueCount: 2,
    issues: [
      {
        ...
        codeSnippet:
          "Parse transaction hashes, timestamps, asset symbols, network names, and amounts.",
      },
      {
        ...
        finding:
          "The artifact operates on high-impact financial data without a verifiable execution boundary.",
      },
    ],
