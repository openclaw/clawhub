...
describe("httpApiV1 handlers", () => {
  ...
  it("get skill returns payload", async () => {
    ...
    expect(json.moderation.verdict).toBe("suspicious");
    expect(json.moderation.reasonCodes).toEqual(["suspicious.dangerous_exec"]);
  });

  it("get skill moderation returns sanitized evidence for public callers", async () => {
    vi.mocked(getOptionalApiTokenUserId).mockResolvedValue(undefined as never);
    const runQuery = vi.fn(async (_query: unknown, args: Record<string, unknown>) => {
      if ("slug" in args) {
        return {
          skill: {
          ...
          latestVersion: null,
          owner: { _id: "users:1", handle: "p", displayName: "Peter", image: null },
          moderationInfo: {
            isPendingScan: false,
            isMalwareBlocked: false,
            isSuspicious: true,
