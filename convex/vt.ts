...
type BackfillModerationV2Result = {
  total: number;
  updated: number;
  errors: number;
  durationMs?: number;
...
/**
 * Backfill normalized moderation fields for all published skills (excluding removed).
 * Safe to rerun: each batch recomputes from version static/vt/llm signals.
 */
export const backfillModerationV2 = internalAction({
  args: {
    cursor: v.optional(v.number()),
    batchSize: v.optional(v.number()),
    accTotal: v.optional(v.number()),
  ...
  handler: async (ctx, args): Promise<BackfillModerationV2Result> => {
    const batchSize = args.batchSize ?? 100;
    const cursor = args.cursor ?? 0;
    const startTime = args.startTime ?? Date.now();
    let accTotal = args.accTotal ?? 0;
    ...
    const batch = await ctx.runQuery(internal.skills.getSkillBatchForModerationBackfillInternal, {
      cursor,
      batchSize,
    });
    ...
    for (const entry of batch.skills) {
      try {
        const result = await ctx.runMutation(internal.skills.applyModerationBackfillInternal, {
          skillId: entry.skillId,
          versionId: entry.versionId,
        });
        if (result?.ok) accUpdated += 1;
      } catch (error) {
        console.error("[vt:backfillModerationV2] Failed for", entry.slug, error);
        accErrors += 1;
      }
    }
    accTotal += batch.skills.length;
    ...
    if (!batch.done) {
      await ctx.scheduler.runAfter(0, internal.vt.backfillModerationV2, {
        cursor: batch.nextCursor,
        batchSize,
        accTotal,
        accUpdated,
      ...
      return { total: accTotal, updated: accUpdated, errors: accErrors, done: false };
    }
    ...
    return { total: accTotal, updated: accUpdated, errors: accErrors, durationMs, done: true };
  },
});
