    ...
    if (obj && typeof obj === "object" && !Array.isArray(obj)) {
      ...
      for (const ns of ["clawdbot", "clawdis", "openclaw"]) {
        ...
        if (nsObj && typeof nsObj === "object" && !Array.isArray(nsObj)) {
          ...
          if (isValidSkillCategory(cat)) return cat;
          return undefined;
        }
      }
...
export function buildModerationSnapshot(params: {
  ...
  // Fail-closed: unless the LLM explicitly confirmed clean/benign, revert
  // security-contextualized codes back to their original suspicious.* form.
  // The static scanner rewrites suspicious.* to info.security_context.* for
  // security-category skills, but that leniency is only safe when the LLM
  // coherence check verified the patterns are legitimate. Missing, errored,
  // pending, or non-clean LLM statuses all trigger reversion.
  if (!isExternalScannerClean(params.llmStatus)) {
    staticCodes = staticCodes.map((code) =>
      code.startsWith("info.security_context.")
        ? code.replace(/^info\.security_context\./, "suspicious.")
        : code,
    );
  }
...
export function resolveSkillVerdict(
  ...
  return verdictFromCodes(skill.moderationReasonCodes ?? []);
}
