...
describe("skillPublish", () => {
  ...
  it("allows official org publishers to publish bundled skill-card.md files", async () => {
    ...
    const runAfter = vi.fn(async () => ({ id: "scheduled" }));
    const ctx = {
      runQuery,
      runMutation,
      scheduler: { runAfter },
        ...
        get: vi.fn(async (storageId: string) => {
          if (storageId === "_storage:skill") {
            return new Blob([
              [
                "---",
                "description: Demo skill with a bundled official card.",
                "---",
                "# Demo",
                "Use this skill to validate bundled Skill Card publishing.",
              ].join("\n"),
            ]);
          }
          return new Blob(["# Skill Card\n\nOfficially authored trust notes."]);
        }),
      },
    };
    ...
    expect(runMutation).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        fingerprint: await __test.buildPublishSourceFingerprint(
          [
            { path: "SKILL.md", sha256: "a".repeat(64) },
            { path: "skill-card.md", sha256: "b".repeat(64) },
          ],
          { includeSkillCard: true },
        ),
        files: expect.arrayContaining([expect.objectContaining({ path: "skill-card.md" })]),
      }),
    );
  });
