...
describe("moderationEngine", () => {
  ...
  describe("security tool category", () => {
    ...
    it("reverts security context codes when LLM is unavailable (fail-closed)", () => {
      const snapshot = buildModerationSnapshot({
        staticScan: {
          status: "clean",
          reasonCodes: ["info.security_context.dangerous_exec", "info.security_context.env_credential_access"],
          findings: [],
          summary: "",
          engineVersion: "v2.1.1",
          checkedAt: Date.now(),
        ...
        vtStatus: "clean",
        // llmStatus intentionally omitted — LLM did not run
      });

      // Without LLM confirmation, security-contextualized codes should
      // revert to their original suspicious.* form for verdict calculation.
      expect(snapshot.reasonCodes).toContain("suspicious.dangerous_exec");
      expect(snapshot.reasonCodes).toContain("suspicious.env_credential_access");
      expect(snapshot.reasonCodes).not.toContain("info.security_context.dangerous_exec");
      expect(snapshot.verdict).toBe("suspicious");
    });

    it("reverts security context codes when LLM errored (fail-closed)", () => {
      const snapshot = buildModerationSnapshot({
        staticScan: {
          status: "clean",
          reasonCodes: ["info.security_context.dangerous_exec"],
          findings: [],
          summary: "",
          engineVersion: "v2.1.1",
          checkedAt: Date.now(),
      ...
      // LLM ran but failed — still not a clean confirmation.
      expect(snapshot.reasonCodes).toContain("suspicious.dangerous_exec");
      expect(snapshot.reasonCodes).not.toContain("info.security_context.dangerous_exec");
      expect(snapshot.verdict).toBe("suspicious");
    });

    it("keeps security context codes when LLM confirms clean", () => {
      const snapshot = buildModerationSnapshot({
        staticScan: {
          status: "clean",
          reasonCodes: ["info.security_context.dangerous_exec"],
      ...
      // With LLM confirmation, contextualized codes stay as info.*
      // and the clearable-code suppression can remove them entirely.
      expect(snapshot.verdict).toBe("clean");
    });
  });
