...
import { isOfficialOrgPublisher } from "./officialPublishers";
...
export async function publishVersionForUser(
  ...
  const hasBundledSkillCard = publishFiles.some((file) => isSkillCardPath(file.path));
  const canPublishSkillCard =
    hasBundledSkillCard && (await canPublishBundledSkillCard(ctx, options.ownerPublisherId));
  if (hasBundledSkillCard && !canPublishSkillCard) {
    throw new ConvexError(
      "skill-card.md is generated by ClawHub and can only be published directly by official org publishers",
    );
  }
  ...
  const fingerprintPromise = buildPublishSourceFingerprint(
    ...
    { includeSkillCard: canPublishSkillCard },
  );
...
async function buildPublishSourceFingerprint(
  files: FingerprintFile[],
  options: { includeSkillCard?: boolean } = {},
) {
  return await hashSkillFiles(
    options.includeSkillCard ? files : files.filter((file) => !isSkillCardPath(file.path)),
  );
}

async function canPublishBundledSkillCard(
  ctx: Pick<ActionCtx, "runQuery">,
  ownerPublisherId: Id<"publishers"> | undefined,
) {
  if (!ownerPublisherId) return false;
