...
Submitted search text must not mint arbitrary public homepage chips. Search telemetry may boost only curated recommendation topic buckets, and unmatched raw terms must remain private aggregate data until they age out. The telemetry write endpoint is an internal UI sink at `/api/v1/search/telemetry`, not part of the public search-read contract, and should accept browser writes only from ClawHub-controlled or local development origins.

Skill and plugin activity signals should be derived from daily stat tables, not all-time totals, so suggestions respond to recent demand without duplicating the visible trending skill section. Plugin signals use package-level daily stats for `code-plugin` and `bundle-plugin` families. The public telemetry write path must stay rate-limited. Recommendation rebuilds and public reads must use bounded, indexed queries rather than scanning full telemetry or stats tables. Search telemetry dedupe rows and aggregate rows are retained only for the short recommendation window plus slack; cleanup must drain expired rows with indexed batches and continuation scheduling.
