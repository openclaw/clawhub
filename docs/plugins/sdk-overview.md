...
<Tip>
Looking for a how-to guide instead? Start with [Building plugins](./building-plugins.md), use [Channel plugins](./sdk-channel-plugins.md) for channel plugins, [Provider plugins](./sdk-provider-plugins.md) for provider plugins, [CLI backend plugins](./cli-backend-plugins.md) for local AI CLI backends, and [Plugin hooks](./hooks.md) for tool or lifecycle hook plugins.
</Tip>
...
<Warning>
  ...
  `runtime-api.ts` barrels; OpenClaw contributors should either use those
  plugin-local barrels or add a narrow generic SDK contract when a need is
  truly cross-channel.
...
[Plugin SDK subpaths](./sdk-subpaths.md).

The installed `openclaw` package's `exports` map is the source of truth for
third-party plugins. ClawHub's bundled Plugin Inspector reports imports that are
deprecated, reserved, internal, or missing from the public package surface:

```bash
clawhub package validate .
```

OpenClaw contributors maintain the generated entrypoint inventory and boundary
reports in the OpenClaw repository. Those repo-local scripts are not required
for external ClawHub packages.
...
Use [`defineToolPlugin`](./tool-plugins.md) for simple tool-only plugins
...
[CLI backend plugins](./cli-backend-plugins.md).
...
See [Plugin hooks](./hooks.md) for examples, common hook names, and guard
semantics.
...
| `api.runtime`            | `PluginRuntime`           | [Runtime helpers](./sdk-runtime.md)                                                         |
...
<CardGroup cols={2}>
  <Card title="Entry points" icon="door-open" href="./sdk-entrypoints.md">
    `definePluginEntry` and `defineChannelPluginEntry` options.
  </Card>
  <Card title="Runtime helpers" icon="gears" href="./sdk-runtime.md">
    Full `api.runtime` namespace reference.
  </Card>
  <Card title="Setup and config" icon="sliders" href="./sdk-setup.md">
    Packaging, manifests, and config schemas.
  </Card>
  <Card title="Testing" icon="vial" href="./sdk-testing.md">
    Test utilities and lint rules.
  </Card>
  <Card title="SDK migration" icon="arrows-turn-right" href="./sdk-migration.md">
    Migrating from deprecated surfaces.
  </Card>
  <Card title="Plugin internals" icon="diagram-project" href="https://docs.openclaw.ai/plugins/architecture">
