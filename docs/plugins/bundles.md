...
## Distribute bundle content through ClawHub

ClawHub does not currently provide a pure ecosystem-bundle publish path.
`clawhub package publish` requires `openclaw.plugin.json`, while OpenClaw treats
a directory containing that manifest as a native plugin instead of a Codex,
Claude, or Cursor bundle. Do not add an OpenClaw manifest only to make a bundle
publishable.

If the bundle primarily contains skills, publish each skill folder separately:

```bash
clawhub skill publish ./my-bundle/skills/my-skill \
  --slug my-skill \
  --name "My Skill" \
  --version 1.0.0
```

For bundle-only MCP, settings, commands, or hook-pack behavior, distribute the
bundle source or archive and use the direct OpenClaw install commands above. To
publish through ClawHub's plugin registry, convert the bundle to a
[native OpenClaw plugin](./building-plugins.md).
...
- [Building Plugins](./building-plugins.md) — create a native plugin
- [Plugin Manifest](./manifest.md) — native manifest schema
