...
<Tip>
**Looking for a walkthrough?** The how-to guides cover packaging in context: [Channel plugins](./sdk-channel-plugins.md#step-1-package-and-manifest) and [Provider plugins](./sdk-provider-plugins.md#step-1-package-and-manifest).
</Tip>
...
fields are required. See [ClawHub publishing](../publishing.md) for the
registry-facing workflow.
</Note>
...
<ParamField path="runtimeExtensions" type="string[]">
  Built JavaScript entry points used by installed packages. Required for
  ClawHub publishing when `extensions` points at TypeScript.
</ParamField>
<ParamField path="setupEntry" type="string">
  Lightweight setup-only entry (optional).
</ParamField>
<ParamField path="runtimeSetupEntry" type="string">
  Built JavaScript peer for `setupEntry` (optional unless `setupEntry` is
  published as TypeScript).
</ParamField>
...
See [Plugin manifest](./manifest.md) for the full schema reference.
...
<Note>
The build must create every file declared by `runtimeExtensions` and
`runtimeSetupEntry`. The legacy skill-only publish alias is for skills. Plugin
packages should always use `clawhub package publish`.
</Note>
...
**External plugins:** publish to [ClawHub](../publishing.md), then install:
...
<Tabs>
  ...
  <Tab title="npm fallback">
    Use npm when a package has not moved to ClawHub yet:
  ...
  <Tab title="Explicit npm">
    Use an explicit npm locator when you need to bypass ClawHub resolution
    during migration:
...
**OpenClaw bundled plugins:** OpenClaw contributors place these under the
OpenClaw repository's bundled plugin workspace tree, where the build discovers
them automatically.

**Users can also install a bare npm package during the launch cutover:**
...
- [Building plugins](./building-plugins.md) — step-by-step getting started guide
- [Plugin manifest](./manifest.md) — full manifest schema reference
- [SDK entry points](./sdk-entrypoints.md) — `definePluginEntry` and `defineChannelPluginEntry`
