...
Use [internal hooks](https://docs.openclaw.ai/automation/hooks) instead when you want a small
...
- `requireApproval` pauses the agent run and asks the user through plugin
  ...
  to the matching app-server approval request; see [Codex harness runtime](https://docs.openclaw.ai/plugins/codex-harness-runtime#hook-boundaries).
...
See [Plugin permission requests](./plugin-permission-requests.md) for
...
- **Plaintext channel envelopes** in `inbound_claim` and `message_received`
  ...
  [Plaintext channel envelopes → BodyForAgent](./sdk-migration.md#active-deprecations).
...
[Plugin SDK migration → Active deprecations](./sdk-migration.md#active-deprecations).

## Related

- [Plugin SDK migration](./sdk-migration.md) - active deprecations and removal timeline
- [Building plugins](./building-plugins.md)
- [Plugin SDK overview](./sdk-overview.md)
- [Plugin entry points](./sdk-entrypoints.md)
