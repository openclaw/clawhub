...
<Info>
  ...
  [provider plugin](./sdk-provider-plugins.md) instead. If the upstream
  runtime owns complete agent sessions, tool events, compaction, or background
  task state, use an [agent harness](https://docs.openclaw.ai/plugins/sdk-agent-harness).
</Info>
...
<Steps>
  <Step title="Create package metadata">
    ...
    Configure your package build to emit `dist/index.js`. Published packages
    must ship every JavaScript file declared by `openclaw.runtimeExtensions`.
    See [Entry points](./sdk-entrypoints.md).
...
For an external plugin, run its tests and ClawHub compatibility validation:
...
OpenClaw contributors working on a bundled plugin should also run the plugin's
...
- [CLI backends](https://docs.openclaw.ai/gateway/cli-backends) - user configuration and runtime behavior
- [Building plugins](./building-plugins.md) - package and manifest basics
- [Plugin SDK overview](./sdk-overview.md) - registration API reference
- [Plugin manifest](./manifest.md) - `cliBackends` and setup descriptors
- [Agent harness](https://docs.openclaw.ai/plugins/sdk-agent-harness) - full external agent runtimes
