...
<Info>
  ...
  [Getting Started](./building-plugins.md) first for the basic package
...
[Channel ingress API](https://docs.openclaw.ai/plugins/sdk-channel-ingress) for the API design,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](./sdk-setup.md#openclaw.channel):
    ...
    The ClawHub baseline fields under `compat` and `build` are required for
    external code plugins. Configure your build to emit both declared
    `runtimeExtensions` and `runtimeSetupEntry` JavaScript files before
    publishing.
  ...
  <Step title="Wire the entry point">
    ...
    [Entry Points](./sdk-entrypoints.md#definechannelpluginentry) for all
    options.
  ...
  <Step title="Add a setup entry">
    ...
    See [Setup and Config](./sdk-setup.md#setup-entry) for details.
...
Write colocated tests in `src/channel.test.ts`:
    ...
    External plugin repositories should run their own test script and ClawHub
    ...
    OpenClaw contributors working on a bundled channel should also run its
    targeted OpenClaw test lane:

    ```bash
    pnpm test -- <bundled-plugin-root>/acme-chat/
    ```

    For the external test workflow and the OpenClaw-only helper reference, see
...
├── dist/
│   ├── index.js              # Built runtime entry published to ClawHub
│   └── setup-entry.js        # Built setup entry published to ClawHub
...
<CardGroup cols={2}>
  <Card title="Threading options" icon="git-branch" href="./sdk-entrypoints.md#registration-mode">
    Fixed, account-scoped, or custom reply modes
  </Card>
  <Card title="Message tool integration" icon="puzzle" href="https://docs.openclaw.ai/plugins/architecture#channel-plugins-and-the-shared-message-tool">
    describeMessageTool and action discovery
  </Card>
  <Card title="Target resolution" icon="crosshair" href="https://docs.openclaw.ai/plugins/architecture-internals#channel-target-resolution">
    inferTargetChatType, looksLikeId, resolveTarget
  </Card>
  <Card title="Runtime helpers" icon="settings" href="./sdk-runtime.md">
    TTS, STT, media, subagent via api.runtime
  </Card>
  <Card title="Channel inbound API" icon="bolt" href="https://docs.openclaw.ai/plugins/sdk-channel-inbound">
...
- [Provider Plugins](./sdk-provider-plugins.md) - if your plugin also provides models
- [SDK Overview](./sdk-overview.md) - full subpath import reference
- [SDK Testing](./sdk-testing.md) - test utilities and contract tests
- [Plugin Manifest](./manifest.md) - full manifest schema

## Related

- [Plugin SDK setup](./sdk-setup.md)
