...
read_when:
  ...
  - Checking whether an SDK import is public for third-party plugins
...
purpose. The installed `openclaw` package's `exports` map is authoritative for
third-party plugins. Before publishing, run `clawhub package validate .` to
catch deprecated, reserved, internal, or unavailable imports.

For the plugin authoring guide, see [Plugin SDK overview](./sdk-overview.md).
...
focused SDK subpaths below. Broad barrels such as `compat`, `config-types`,
...
- [Plugin SDK overview](./sdk-overview.md)
