...
External ClawHub plugins should test their own behavior, validate the package
contract, and prove the installed runtime. OpenClaw also has repo-local helpers
for its bundled plugins; those helpers are documented later on this page but
are not public package exports for third-party plugins.
...
tests for their external systems. See [Plugin Inspector](../plugin-inspector.md)
for repeatable scripts, runtime capture, reports, and CI integration.

<Tip>
  **Looking for test examples?** The how-to guides include worked test examples:
  [Channel plugin tests](./sdk-channel-plugins.md#step-6-test) and
  [Provider plugin tests](./sdk-provider-plugins.md#step-6-test).
</Tip>
...
they may import Vitest or other repo-only test dependencies. Do not copy these
imports into a package published on ClawHub.
...
- [SDK Overview](./sdk-overview.md) -- import conventions
- [SDK Channel Plugins](./sdk-channel-plugins.md) -- channel plugin interface
- [SDK Provider Plugins](./sdk-provider-plugins.md) -- provider plugin hooks
- [Building Plugins](./building-plugins.md) -- getting started guide
